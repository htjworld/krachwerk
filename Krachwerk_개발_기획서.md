...
(2026-09 v2 갱신: 아래는 최초 초안이고 실제 구현은 이보다 훨씬 자세하다 — 전체 스펙은
`_research/`의 세션 명세서, 요약은 README.md 참고.)

리듬 그리드: 16스텝 시퀀서. 시드는 우선 정규 코드(base36 8자)로 정규화되고, Feistel
치환을 거쳐 9개 필드로 된 "게놈"으로 풀린다 — 어느 편곡 계열을 쓸지, 템포(104~132
BPM 범위 안에서), 조성, 곡 길이(150~210초), 드럼 변형 등을 전부 이 게놈이 결정한다.
"킥은 스텝 0/4/8/12 고정, 하이햇은 항상 재생"은 세 가지 기존 템플릿에만 해당하고,
Kraftwerk 두 곡을 분석해 만든 compute/metropolis 계열(전체의 80%)은 마디마다 타격
밀도가 늘었다 줄었다 하는 훨씬 복잡한 리듬을 쓴다 — 이게 원래 "박자가 안 쪼개진다"는
피드백을 고치려고 v2에서 새로 만든 부분이다.
...
시드 입력을 문자열로 받아 정규 코드(base36 8자, `36^8`가지)로 정규화한 뒤 Feistel
치환으로 섞어서 내부 생성 로직에 넣는다. 사용자가 입력할 수 있는 원문 문자열의
가짓수는 사실상 무한이고, 치환이 전단사(bijective)라서 **서로 다른 정규 코드는
게놈 필드가 최소 하나는 반드시 다르다** — "완전히 같은 곡이 나오는 두 시드"가 우연이
아니라 구조적으로 없다는 뜻이다(기존 계획: 단순 해시 → PRNG. 실제로는 치환 기반
유일성 보장으로 더 강화했다).
